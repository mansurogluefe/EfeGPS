...
function loadRoute(dateStr) {
    ...
    // Tarihi başlangıç ve bitiş timestamp'lerine çevir
    ...
    tripsRef.orderByChild('startTime').startAt(startOfDay).endAt(endOfDay).once('value', (snapshot) => {
        const points = [];
        if (snapshot.exists()) {
            snapshot.forEach((tripSnap) => {
                const tripData = tripSnap.val();
                if (tripData.points) {
                    Object.values(tripData.points).forEach(p => {
                        points.push({
                            lat: parseFloat(p.lat),
                            lng: parseFloat(p.lng),
                            speed: parseFloat(p.speed || 0),
                            bearing: parseFloat(p.bearing || 0),
